...
def _build_performance_md(
    *,
    ...
    drop_rate_paths: list[Path] | None,
    timestamp: str,
) -> str:
    ...
    matching comparison, drop-rate, and CDF PNGs (when diagrams were
    generated). Per-QoS embed order (T16.14): receive throughput +
    latency (``comparison-qos<N>.png``), drop rate
    (``drop-rate-qos<N>.png``), latency CDF
    (``latency-cdf-qos<N>.png``). Image links are relative to the
    markdown file (so the file is portable as long as the PNGs sit
    alongside it -- the default ``output_dir`` layout). If a QoS has
    no diagram (diagrams were not generated this invocation) the
    image section is skipped silently so the file stays useful as a
    tables-only dump.
    """
    ...
    if not comparison_paths and not cdf_paths and not drop_rate_paths:
        return body

    # Group PNG paths by QoS so the markdown renders one section per
    # observed QoS level. Comparison, drop-rate, and CDF filenames
    # share the same ``-qos<N>`` segment so the same numeric rank
    # groups them.
    ...
    for p in drop_rate_paths or []:
        rank = _qos_rank_for_image_path(p)
        grouped.setdefault(rank, {})["drop_rate"] = p
    ...
    for rank in sorted(grouped.keys()):
        if rank >= 10**9:
            header = "## Legacy spawns (no QoS) - receive throughput, drop rate, CDF"
        else:
            header = f"## QoS {rank} - receive throughput + latency, drop rate, CDF"
        lines = [header, ""]
        section = grouped[rank]
        # T16.14 per-QoS embed order: receive throughput + latency,
        # then drop rate, then latency CDF. Keep the order even when
        # individual PNGs are missing (e.g. drop_rate but no cdf).
        if "comparison" in section:
            lines.append(f"![]({section['comparison'].name})")
            lines.append("")
        if "drop_rate" in section:
            lines.append(f"![]({section['drop_rate'].name})")
            lines.append("")
        if "cdf" in section:
...
def _write_dump_files(
    *,
    ...
    drop_rate_paths: list[Path] | None = None,
) -> list[Path]:
    ...
    ``comparison_paths``/``drop_rate_paths``/``cdf_paths`` are
    forwarded to ``_build_performance_md`` so the per-QoS PNGs
    generated by
    ``generate_comparison_plot``/``generate_drop_rate_plot``/``generate_latency_cdf_plot``
    get embedded under per-QoS markdown headers in
    ``summary_performance.md`` -- T16.13 / T16.14.
    """
    ...
    performance_path.write_text(
        _build_performance_md(
            ...
            cdf_paths=cdf_paths,
            drop_rate_paths=drop_rate_paths,
            timestamp=timestamp,
        ),
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # under per-QoS headers -- T16.13 / T16.14.
        ...
        if do_diagrams:
            ...
            drop_rate_paths = generate_drop_rate_plot(performance_results, output_dir)
            for p in drop_rate_paths:
                print(f"Plot saved to: {p}", file=sys.stderr)
        ...
        if do_summary:
            ...
            performance_md_path.write_text(
                _build_performance_md(
                    ...
                    drop_rate_paths=drop_rate_paths if do_diagrams else None,
                    timestamp=timestamp,
                ),
                encoding="utf-8",
            )
