...
fn run() -> Result<()> {
    ...
    let buffer_size_str = parse_required_extra_arg(&args.extra, "buffer-size")
        .context("missing required --buffer-size in variant-specific args")?;
    let buffer_size: usize = buffer_size_str
        .parse()
        .with_context(|| format!("invalid --buffer-size: {}", buffer_size_str))?;
    ...
    // control side-channel. QoS-independent (`runner_stride = 1`, no
    // QoS stride); see CUSTOM.md "Control side-channel (T14.18)".
...
///
/// One control listen port per (runner, variant binary). QoS-
/// independent. Pairing: lower-sorted-name peer listens; higher-
/// sorted peer connects.
#[derive(Debug, Clone)]
...
/// Derive per-runner control TCP endpoints.
///
/// Convention:
///   runner_stride = 1
...
/// Hybrid / QUIC variants):
///   runner_stride = 1
///   qos_stride    = 10
///   port = tcp_base_port + runner_index * runner_stride + (qos - 1) * qos_stride
///
/// The TCP endpoints are only USED at QoS 4, but derivation must SUCCEED at all
/// QoS levels so that the parse step doesn't fail before the variant decides
...
mod tests {
    ...
    fn test_parse_extra_arg_not_found() {
        let extra: Vec<String> = vec!["--tcp-base-port".into(), "19800".into()];
    ...
    fn test_identity_resolution_alice_index_0() {
        ...
        let derived = derive_endpoints(&peers, "alice", 19800, 1).unwrap();
        // alice index 0, qos 1: 19800 + 0 + 0 = 19800
        assert_eq!(derived.tcp_listen_addr.port(), 19800);
        assert_eq!(derived.tcp_peers.len(), 1);
        // bob index 1, qos 1: 19800 + 1 + 0 = 19801
        assert_eq!(derived.tcp_peers[0].port(), 19801);
    }

    #[test]
    fn test_identity_resolution_bob_index_1() {
        let peers = parse_peers("alice=127.0.0.1,bob=127.0.0.1").unwrap();
        let derived = derive_endpoints(&peers, "bob", 19800, 1).unwrap();
        assert_eq!(derived.tcp_listen_addr.port(), 19801);
        assert_eq!(derived.tcp_peers.len(), 1);
        assert_eq!(derived.tcp_peers[0].port(), 19800);
    }

    #[test]
    fn test_port_derivation_qos4_runner1() {
        // base 19800, runner_index 1, qos 4 -> 19800 + 1*1 + 3*10 = 19831
        let peers = parse_peers("alice=127.0.0.1,bob=127.0.0.1").unwrap();
        let derived = derive_endpoints(&peers, "bob", 19800, 4).unwrap();
        assert_eq!(derived.tcp_listen_addr.port(), 19831);
        // alice (index 0) at qos 4: 19800 + 0 + 30 = 19830
        assert_eq!(derived.tcp_peers[0].port(), 19830);
    }
    ...
    fn test_port_derivation_all_qos_levels_disjoint() {
        ...
        for runner in ["alice", "bob"] {
            for qos in 1..=4u8 {
                let d = derive_endpoints(&peers, runner, 19800, qos).unwrap();
                assert!(
    ...
    fn test_runner_not_in_peers_errors() {
        let peers = parse_peers("alice=127.0.0.1,bob=127.0.0.1").unwrap();
        let err = derive_endpoints(&peers, "carol", 19800, 1).unwrap_err();
        let msg = format!("{err:#}");
        assert!(
    ...
    fn test_invalid_qos_errors() {
        ...
        assert!(derive_endpoints(&peers, "a", 19800, 0).is_err());
        assert!(derive_endpoints(&peers, "a", 19800, 5).is_err());
    }

    #[test]
    fn test_self_only_no_peers_to_connect() {
        let peers = parse_peers("self=127.0.0.1").unwrap();
        let derived = derive_endpoints(&peers, "self", 19800, 1).unwrap();
        assert_eq!(derived.tcp_listen_addr.port(), 19800);
        assert!(derived.tcp_peers.is_empty());
    }

    #[test]
    fn test_derive_endpoints_succeeds_at_all_qos() {
        // Even at QoS 1-3 (where TCP isn't used) the derivation must succeed
        // because the parser runs unconditionally before the variant decides
        // which transport path to wire up.
        let peers = parse_peers("self=127.0.0.1").unwrap();
        for qos in 1..=4u8 {
            assert!(
                derive_endpoints(&peers, "self", 19800, qos).is_ok(),
                "derive_endpoints failed at qos {qos}"
            );
        }
    }
    ...
    fn control_endpoints_no_qos_stride() {
        let peers = parse_peers("alice=127.0.0.1,bob=127.0.0.1").unwrap();
        // Calling twice with the same arguments yields the same port -- no
        // hidden state mutated by QoS context.
