//! Compact binary message format used inside the WebSocket binary frame body.
//!
//! Mirrors the wire format used by `variants/hybrid` and `variants/custom-udp`:
//! the same bytes are produced by both sides, so a single decoder logic is
//! used cross-variant. WebSocket framing replaces the 4-byte length prefix
//! used on the raw TCP path: the length is carried by the WS frame header.
//!
...
/// Encode a data update into the compact binary format used in the
...
mod tests {
    ...
    fn decode_frame_dispatches_data_vs_eot() {
        let data_buf = encode_data(Qos::ReliableTcp, 7, "/p", "w", &[1, 2]);
